...
新用户更推荐用 `cytopert setup` 走交互式配置向导（厂商 / API key / 模型 / workspace 一次问完，并在保存前做一次 1-token 的连通性测试）。任何时候都可以用 `cytopert doctor` 跑一遍 PASS/WARN/FAIL 体检：profile、配置、provider key、workspace 可写、`state.db` + FTS5、bundled skills、plugins、scanpy、decoupler 全覆盖。如果同时跑多个研究项目，建议用 profile 隔离：`cytopert profile new <name>` 创建，`cytopert -p <name> ...` 单次切换，`cytopert profile use <name>` 持久化为默认。每个 profile 都有独立的 `config.json` / `state.db` / `workspace` / `memory` / `chains` / `skills` / `sessions` / `jobs.json`。
...
`CYTOPERT_HOME` 环境变量可以改根目录，方便给不同课题做隔离。更系统的做法是用 profile：`cytopert profile new study42 && cytopert profile use study42`，每个 profile 在 `~/.cytopert/profiles/<name>/` 下持有完整且独立的状态树（含 `state.db`、`workspace`、`memory`、`chains`、`skills`、`sessions`、`jobs.json`）。

`cytopert cron` 可以让某条消息或 scenario 按 `every Nm/Nh/Nd` 或 `hourly` / `daily` 这样的简单语法定时触发——`tick` 模式跑一遍当前到点的任务退出（可挂到系统 cron 上），`daemon` 模式则进 sleep 循环常驻。`cytopert skills install <git-url|archive|本地目录>` 把第三方 SKILL.md 拉到当前 profile 的 `skills/` 下，配合 `skills search` / `skills uninstall` 形成最小可用的 skills hub。
