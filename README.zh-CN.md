...
**结论挂证据。** Agent 的每个机理结论都必须引用 evidence ID，证据来自工具产出（如 DE 表）。没有证据就不能下结论，agent 会停下来要数据，不会编造引用。注：早期文档曾提到"被注册的文献"作为知识类证据来源；这一通道在阶段 1 之后还没有正式实现，目前只有数据类证据（`EvidenceType.DATA`）会进库。`EvidenceType.KNOWLEDGE` 将在阶段 7.2 的 `pathway_lookup` 工具上线后投入使用。
...
不真正跑分析），在阶段 1 整改时已经被从注册表中移除。真实接入 decoupler PROGENy /
DoRothEA / CollecTRI 的 `pathway_lookup` 将在阶段 7.2 上线。
